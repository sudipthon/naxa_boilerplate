...
from .models import PlacesCoordinate as Places
from .serializers import DistrictAreaSerializer, FileUploadSerializer, PlacesSerializer
    ...
    def nearby_place(self, request):
        ...
        if place_name:
            ...
            if place_:
                ...
                self.queryset = Places.objects.filter(
                    location__distance_lt=(place_.location, D(m=10000))
                ).exclude(id=place_.id)
...
class CalculateDistanceAPIView(APIView):
    ...
    def post(self, request):
        ...
        if place1 and place2:
            place1_long, place1_lat = float(place1["long"]), float(place1["lat"])
            place2_long, place2_lat = float(place2["long"]), float(place2["lat"])
        ...
        return Response({"message": f"distance is {distance_km}  km", "status": 200})
...
class DistrictAreaViewset(
    ...
    def calculate_if_point_in_district(self, request):
        long, lat = request.data.get("long", None)
        lat = request.data.get("lat", None)
        district_name = request.data.get("district_name", None)
        if not all([long, lat, district_name]):
            return Response(
                {"message": "long,lat and district_name are required", "status": 400}
            )

        district = DistrictArea.objects.filter(name__icontains=district_name).first()

        point = Point(float(long), float(lat))
        districts = district.area.contains(point)
        if districts:
            return Response(
                {"message": f"Point is in {district.name} district", "status": 200}
            )
        else:
            return Response(
                {"message": f"Point is not in {district.name} district", "status": 200}
            )
    ...
    def add_districts_file(self, request):
        ...
        return Response({"message": "Adding districts in background", "status": 200})
