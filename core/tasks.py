...
def add_districts(file):
    ...
    try:
        with default_storage.open(file, 'rb') as f:
            ...
            data = pd.read_csv(f)
        objs_to_create = [DistrictArea(
                name=row["name"], area=row["area"]) for index, row in data.iterrows()]
        created_objs = DistrictArea.objects.bulk_create(
                objs_to_create, batch_size=100)
